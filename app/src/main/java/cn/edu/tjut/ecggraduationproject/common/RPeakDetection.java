package cn.edu.tjut.ecggraduationproject.common;
...
public class RPeakDetection {
	...
	public int[] RPeakLocationRecognize(float[] data)//判断R点在数据中的数组下标
	{
		if (null == data)
		{
			return null;
		}
		ArrayList<Integer> ExtremelocationTemp = new ArrayList<Integer>();  //存放R点下标的临时列表
				...
				if (Math.abs(data[i] - data[nMaxExtentPos]) > nMaxExtent / 2 || nMaxExtent > nRHeight / 2) //第一个急剧下降的点，或高度> nRHeight / 2
				{
					if (nMaxExtent > nRHeight / 2)//设置高度
					{
						if ((nRWaveCount == 0) || (i - ExtremeTemp.get(nRWaveCount - 1)) > nRWidth)//第一个波形,或者超出一个波形的宽度,即采样点个数
						{
							...
							ExtremelocationTemp.add(i);//将R点下标添加到locationlist里
		...
		int[] RpeakPointLocationData = new int[ExtremelocationTemp.size()];
		for(int j = 0; j < ExtremelocationTemp.size(); j++){
			RpeakPointLocationData[j] = ExtremelocationTemp.get(j);
		}
		return RpeakPointLocationData;
	}
